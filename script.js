    ...
    updateCoins(opponent) {
        for (let i = this.activeCoins.length - 1; i >= 0; i--) {
            ...
            if (
                ...
                coinBox.y + coinBox.height > opponent.y
            ) {
...
function initGame() {
    ...
    // Establece el fondo del canvas para la pelea
    const possibleBgs = [
    ...
    setTimeout(() => {
        startMessageOverlay.classList.add('fade-out'); // Esto activa la transición de opacidad a 0
        
        // Escucha el evento 'transitionend' para saber cuándo la transición ha terminado
        startMessageOverlay.addEventListener('transitionend', function handler() {
            startMessageOverlay.classList.add('hidden'); // Oculta el elemento completamente
            startMessageOverlay.removeEventListener('transitionend', handler); // Elimina el listener para evitar múltiples ejecuciones
...
continueButton.addEventListener('click', () => {
    ...
    // Retrasar la configuración de la pantalla de selección
