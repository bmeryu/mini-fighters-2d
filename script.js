...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class Player {
        ...
        updateProjectiles(opponent, type) {
            let projectiles = [];
            let checkCollision = (p, o) => false;
            let onHit = (p) => {};

            switch(type) {
                case 'piranha': 
                    projectiles = this.activePiranhaProjectiles;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(p.damage, p.direction); activeHitEffects.push({ text: "¡ÑAM!", x: p.x, y: p.y, color: "#ff6347", alpha: 1.0, size: 20, rotation: 0, lifetime: HIT_EFFECT_LIFETIME / 2 }); };
                    break;
                case 'moneyWad':
                    projectiles = this.activeMoneyWads;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(p.damage, p.x > opponent.x + opponent.width / 2); activeHitEffects.push({ text: "$$$", x: p.x, y: p.y, color: "#22c55e", alpha: 1.0, size: 30, rotation: 0, lifetime: HIT_EFFECT_LIFETIME }); };
                    break;
                 case 'coin':
                    projectiles = this.activeCoins;
                    checkCollision = (p, o) => { const coinBox = { x: p.x - p.radius, y: p.y - p.radius, width: p.radius * 2, height: p.radius * 2 }; return coinBox.x < o.x + o.width && coinBox.x + coinBox.width > o.x && coinBox.y < o.y + o.height && coinBox.y + coinBox.height > o.y; };
                    onHit = (p) => { opponent.takeDamage(p.damage, p.x > opponent.x + opponent.width / 2); activeHitEffects.push({ text: "$", x: p.x, y: p.y, color: "#facc15", alpha: 1.0, size: 20, rotation: 0, lifetime: HIT_EFFECT_LIFETIME }); };
                    break;
                case 'calculator':
                    projectiles = this.activeCalculators;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(p.damage, p.velocityX > 0); activeHitEffects.push({ text: "ERROR", x: p.x, y: p.y, color: "#e53e3e", alpha: 1.0, size: 25, rotation: 0, lifetime: HIT_EFFECT_LIFETIME }); };
                    break;
                case 'paper':
                    projectiles = this.activePapers;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(PAPELUCHO_PAPER_DAMAGE, this.facingRight); opponent.isStunned = true; opponent.stunTimer = PAPELUCHO_STUN_DURATION; };
                    break;
                case 'kiss':
                    projectiles = this.activeKisses;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(p.damage, p.direction); activeHitEffects.push({ text: "♥", x: p.x, y: p.y, color: "#ff69b4", alpha: 1.0, size: 30, rotation: 0, lifetime: HIT_EFFECT_LIFETIME }); };
                    break;
                case 'teddy':
                    projectiles = this.activeTeddies;
                    checkCollision = (p, o) => p.x < o.x + o.width && p.x + p.width > o.x && p.y < o.y + o.height && p.y + p.height > o.y;
                    onHit = (p) => { opponent.takeDamage(p.damage, this.facingRight); activeHitEffects.push({ text: "¡AWW!", x: p.x, y: p.y, color: "#9b59b6", alpha: 1.0, size: 40, rotation: 0, lifetime: HIT_EFFECT_LIFETIME }); };
                    break;
            }

            for (let i = projectiles.length - 1; i >= 0; i--) {
                const p = projectiles[i];
                if(type === 'piranha' || type === 'kiss') p.x += p.velocityX * (p.direction ? 1 : -1);
                else { p.y += (p.velocityY += GRAVITY * (type === 'coin' ? 0.6 : type === 'calculator' ? 0.4 : type === 'paper' ? 0.3 : 0.5)); p.x += p.velocityX || 0; }
                if(p.rotationSpeed) p.rotation += p.rotationSpeed;
                p.lifespan--;

                if (p.lifespan <= 0 || p.y > CANVAS_HEIGHT || p.x > CANVAS_WIDTH || p.x + (p.width||0) < 0) {
                    projectiles.splice(i, 1);
                    continue;
                }

                if (!opponent.isSwallowed && !opponent.isStunned && checkCollision(p, {x: opponent.x, y: opponent.y, width: opponent.width, height: opponent.height})) {
                    onHit(p);
                    projectiles.splice(i, 1);
                }
            }
        ...
        update() {
            ...
            if (this.isInvisible) {
                ...
                if (this.invisibilityTimer <= 0) {
                    this.isInvisible = false;
                    const opponent = players.find(p => p !== this);
                    if (opponent) { this.x = opponent.x + (opponent.facingRight ? opponent.width + 20 : -this.width - 20); this.y = opponent.y; }
                }
                return;
            }
            ...
            const opponent = players.find(p => p !== this);
            if (opponent) {
                this.updateProjectiles(opponent, 'piranha');
                this.updateProjectiles(opponent, 'moneyWad');
                this.updateProjectiles(opponent, 'coin');
                this.updateProjectiles(opponent, 'calculator');
                this.updateProjectiles(opponent, 'paper');
                this.updateProjectiles(opponent, 'kiss');
                this.updateProjectiles(opponent, 'teddy');
        ...
        launchSuperpower() {
            ...
            switch(this.name) {
                case "Piraña":
                    new Audio('audio/hadouken.wav').play().catch(e => {});
                    for (let i = 0; i < PIRANHA_PROJECTILE_COUNT; i++) this.activePiranhaProjectiles.push({ x: handX, y: handY - PIRANHA_PROJECTILE_HEIGHT / 2 + (i * 15), width: PIRANHA_PROJECTILE_WIDTH, height: PIRANHA_PROJECTILE_HEIGHT, velocityX: PIRANHA_PROJECTILE_SPEED, direction: this.facingRight, lifespan: PIRANHA_PROJECTILE_LIFESPAN, damage: PIRANHA_PROJECTILE_DAMAGE });
                    return false; // No damage on punch
                case "La Ex":
                    if(opponent) for (let i = 0; i < MONEY_RAIN_COUNT; i++) { const clusterCenterX = opponent.x + opponent.width / 2 + (Math.random() - 0.5) * opponent.width; for (let j = 0; j < 3; j++) this.activeMoneyWads.push({ x: clusterCenterX + (Math.random() - 0.5) * 40, y: MONEY_RAIN_INITIAL_Y - (Math.random() * CANVAS_HEIGHT / 2), width: MONEY_RAIN_WAD_WIDTH, height: MONEY_RAIN_WAD_HEIGHT, velocityX: (Math.random() - 0.5) * 2, velocityY: Math.random() * 4 + 3, rotation: (Math.random() - 0.5) * Math.PI, rotationSpeed: (Math.random() - 0.5) * 0.1, damage: MONEY_RAIN_DAMAGE, lifespan: 200 }); for (let k = 0; k < 5; k++) this.activeCoins.push({ x: clusterCenterX + (Math.random() - 0.5) * 60, y: MONEY_RAIN_INITIAL_Y - (Math.random() * CANVAS_HEIGHT / 2), radius: Math.random() * 5 + 5, velocityY: Math.random() * 5 + 4, damage: COIN_RAIN_DAMAGE, lifespan: 200 }); }
                    return false;
                case "Burric":
                    if(opponent) for (let i = 0; i < CALCULATOR_PROJECTILE_COUNT; i++) this.activeCalculators.push({ x: opponent.x + (Math.random() - 0.5) * opponent.width, y: CALCULATOR_INITIAL_Y - (Math.random() * 100), width: CALCULATOR_PROJECTILE_WIDTH, height: CALCULATOR_PROJECTILE_HEIGHT, velocityX: (Math.random() - 0.5) * 2, velocityY: (Math.random() * 2) + 2, rotation: (Math.random() - 0.5) * 2, rotationSpeed: (Math.random() - 0.5) * 0.2, lifespan: CALCULATOR_PROJECTILE_LIFESPAN, damage: CALCULATOR_PROJECTILE_DAMAGE });
                    return false;
                case "Matthei Bolt":
                    this.isDashing = true; this.dashCount = BOLT_DASH_COUNT; this.dashTargetX = this.x > CANVAS_WIDTH / 2 ? 0 : CANVAS_WIDTH - this.width; this.dashDamageApplied = false; this.velocityY = 0; this.trail = [];
                    return false;
                case "El Zanjas":
                    if(opponent) { this.isCastingCrack = true; this.crackTimer = ZANJAS_CRACK_LIFESPAN; this.crackOpponentHit = false; this.crackCenterX = opponent.x + opponent.width / 2; }
                    return false;
                case "Carolina Papelucho":
                     if(opponent) for (let i = 0; i < PAPELUCHO_PAPER_COUNT; i++) this.activePapers.push({ x: opponent.x + (opponent.width / 2) + (Math.random() - 0.5) * (opponent.width * 2.5), y: -PAPELUCHO_PAPER_HEIGHT - Math.random() * 300, width: PAPELUCHO_PAPER_HEIGHT, height: PAPELUCHO_PAPER_HEIGHT, velocityX: (Math.random() - 0.5) * 4, velocityY: (Math.random() * 2) + 2, rotation: (Math.random() - 0.5) * 2, rotationSpeed: (Math.random() - 0.5) * 0.2, lifespan: 250, isPowerPoint: i === 0 });
                    return false;
                case "Orsini Love":
                    ...
                    return false;
                case "Escape Room Jackson":
                    if(opponent) { this.isInvisible = true; this.invisibilityTimer = JACKSON_INVISIBILITY_DURATION; opponent.isConfused = true; opponent.confusionTimer = JACKSON_CONFUSION_DURATION; new Audio('audio/smoke-bomb.wav').play().catch(e => {}); for (let i = 0; i < SMOKE_PARTICLE_COUNT; i++) smokeParticles.push({ x: this.x + this.width / 2, y: this.y + this.height / 2, radius: Math.random() * 20 + 10, alpha: 1, velocityX: (Math.random() - 0.5) * 4, velocityY: (Math.random() - 0.5) * 4 }); this.x = -1000; }
                    return false;
                case "Tía Cote":
                    if(opponent) for (let i = 0; i < TIA_COTE_TEDDY_COUNT; i++) this.activeTeddies.push({ x: opponent.x + (opponent.width / 2) - (TIA_COTE_TEDDY_WIDTH / 2), y: TIA_COTE_TEDDY_INITIAL_Y - (Math.random() * 50), width: TIA_COTE_TEDDY_WIDTH, height: TIA_COTE_TEDDY_HEIGHT, velocityY: 1.5, rotation: (Math.random() - 0.5) * 0.1, rotationSpeed: (Math.random() - 0.5) * 0.02, damage: TIA_COTE_TEDDY_DAMAGE, lifespan: 200 });
                    return false;
                default:
                    new Audio('audio/38H.wav').play().catch(e => {});
                    return true; // Generic super attack
            }
        }
        ...
        _performAttack(isKickMove) {
            ...
            if (performMeele) {
                ...
                if(isKickMove) {
                    ...
                    const hipY = this.y + this.height - this.shoeHeight - this.thighHeight - this.lowerLegHeight;
                    const hipX = this.x + this.width/2;
                    const attackEndX = hipX + Math.cos(angleOfAttack) * (this.thighHeight+this.lowerLegHeight);
                    const attackEndY = hipY + Math.sin(angleOfAttack) * (this.thighHeight+this.lowerLegHeight);
